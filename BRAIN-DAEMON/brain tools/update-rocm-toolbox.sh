#!/usr/bin/env bash
# update-rocm-toolbox.sh — Met à jour llama.cpp dans le toolbox rocm-7.2

set -euo pipefail

# ── Config ────────────────────────────────────────────────────────────────────
TOOLBOX_NAME="llama-rocm-7.2"
...
DOCKERFILE="$HOME/amd-strix-halo-toolboxes/toolboxes/Dockerfile.rocm-7.2"
...
1)
    ...
    warn "Le build va cloner le dernier master de llama.cpp + télécharger ROCm 7.2 (~30-45 min)."
    ...
    new_ver=$(podman run --rm \
        --device /dev/dri --device /dev/kfd \
        --group-add video --group-add render \
        "$LOCAL_IMAGE" llama-server --version 2>/dev/null \
        | grep -oP 'version: \K[0-9]+' || echo "inconnue")
...
2)
    ...
    new_ver=$(podman run --rm \
        --device /dev/dri --device /dev/kfd \
        --group-add video --group-add render \
        "$UPSTREAM_IMAGE" llama-server --version 2>/dev/null \
        | grep -oP 'version: \K[0-9]+' || echo "inconnue")
    ok "Nouvelle version : build ${BOLD}$new_ver${NC}"
...
5)
    ...
    podman images | grep -E "(REPOSITORY|kyuz0|llama-rocm)" || echo "  Aucune"
