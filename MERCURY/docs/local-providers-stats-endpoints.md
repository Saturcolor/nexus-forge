...
Toutes les routes sont sous le **middleware** OpenRouter, préfixe **`/admin`**.  
Auth :
- en mode standard : **`Authorization: Bearer <admin_token>`** si `admin_token` est configuré ;
- en **`local_only_mode: true`** (défaut), une clé user valide (`users[].api_key`) est aussi acceptée sur `/admin/*` si `admin_accept_user_api_key: true`.

## Mode local-only (recommandé en LAN)

Pour un setup 100% local (sans OpenRouter web) :

- `local_only_mode: true` (désactive le backend/fallback OpenRouter web)
- `openrouter_enabled: false`
- `openrouter_fallback_model: ""`
- `openrouter_api_key: ""` (facultatif mais recommandé pour éviter les ambiguïtés)

Dans Mastermind, vous pouvez séparer :
- `apiKey` : clé utilisée pour les appels provider `/v1/*`
- `statsApiKey` : clé utilisée pour les appels stats `/admin/*` (fallback automatique sur `apiKey` si vide)
