"""Collecte des métriques système (CPU, mémoire, température, GPU, réseau, uptime)."""
import glob
import logging
import time
...
def _get_temperatures() -> Dict[str, Optional[float]]:
    """Températures par capteur : CPU (k10temp), GPU (amdgpu), NVMe."""
    ...
    try:
        ...
        logger.debug("Températures non disponibles: %s", e)
    return result
...
def _get_gpu_amd() -> Dict[str, Any]:
    ...
    if not paths:
        return result
    base = paths[0].rsplit("/", 1)[0]  # /sys/class/drm/cardX/device
...
def _get_network() -> Dict[str, Optional[float]]:
    """Total réseau reçu/envoyé en Mo."""
    try:
...
def collect_system_stats() -> Dict[str, Any]:
    """Retourne CPU, mémoire, températures, GPU, uptime, réseau."""
    try:
        ...
        logger.warning("Erreur collecte système: %s", e)
        return {
            "cpu_percent": None,
